node {
    ...
    stage('Push image') {
     ...
     stage('Kubernetes Setup'){
         sshagent(['instance-2']) {
            sh "scp -o StrictHostKeyChecking=no services.yaml pods.yaml quickstart-kibana.yaml ConfigMap.yaml deployment.yaml seles2112@35.224.11.222:/home/seles2112/"
            script{
                try{
