node {
    ...
    stage('Push image') {
     ...
     stage('Kubernetes Setup'){
         sshagent(['instance-2']) {
            echo "in instance-2"
            sh "scp -o StrictHostKeyChecking=no services.yml pods.yml quickstart-kibana.yaml seles2112@35.239.96.97:/home/seles2112/"
            echo "before scripts"
            script{
                try{
                    ...
                    sh "ssh seles2112@35.239.96.97  kubectl apply -f https://download.elastic.co/downloads/eck/1.0.0/all-in-one.yaml"
