node {
    ...
    stage('Push image') {
     ...
     stage('Kubernetes Setup'){
         sshagent(['instance-1']) {
            echo "in instance-1"
            sh "scp -o StrictHostKeyChecking=no services.yml pods.yml seles2112@104.154.211.43:/home/seles2112/"
            echo "before scripts"
            script{
                try{
                    ...
                    sh "ssh seles2112@104.154.211.43 kubectl apply -f ."
            ...
            // notify("Something failed Kubernetes Setup")
                    ...
                    echo "created"
                    //sh "ssh seles2112@instance-1 kubectl apply -f ."
                    sh "ssh seles2112@104.154.211.43 kubectl create -f ."
            // throw e;
                }
            }  
         }  
